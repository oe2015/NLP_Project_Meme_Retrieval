...
def validate_model(model, processor, val_loader, device, top_k=5):
    ...
    with torch.no_grad():
        for images, img_captions, meme_captions in val_loader:  # Ignore meme_captions
            ...
            img_caption_inputs = processor.tokenizer(img_captions, return_tensors="pt", padding=True, truncation=True, max_length=77).to(device)
...
for epoch in range(50):  # Number of epochs
    ...
    if val_accuracy > best_val_accuracy:
        ...
        checkpoint_path = f"best_checkpoint_meme_captions_new_last_epoch_new_clip_{epoch + 1}.pt"
